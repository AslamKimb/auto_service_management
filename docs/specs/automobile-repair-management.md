...
- **Customer Vehicle**: registration number, customer, selectable Vehicle Make and Vehicle Model, year, VIN/chassis, engine number, engine model, color, fuel type, transmission, current odometer, last service date, warranty, insurer, and notes. Make/model remain optional for legacy and unusual vehicles; when supplied, the selected model must belong to the selected make. Registration and VIN are unique when supplied; search includes registration, VIN/chassis, engine, and customer. Engine number remains the vehicle's serial identifier; engine model links to the controlled Vehicle Engine master.
...
- **Vehicle Engine**: controlled engine-model master with a unique engine code, display name, and optional technical notes. It is referenced by Customer Vehicle and fitment rows; it does not replace the engine serial number.
- **Item Vehicle Fitment**: many-to-many compatibility rows linking one ERPNext Item to an optional make, model, engine model, and year range, with Provisional/Verified status, source, notes, and duplicate-key validation. One physical spare part remains one ERPNext Item; do not create Item Variants for vehicle applications.
...
Each `Repair Job Service` owns Parts, Labour, and Consumables child tables. Service eligibility is not controlled by a user-facing status field; billing, stock, and downstream automation derive from the submitted service content, workshop bay, and parent Repair Job state. Stock rows record optional Item, quantity, UOM, warehouse, selling and cost rates, discounts, amounts, and requested/issued quantities. Part rows compute a server-side fitment match snapshot from the linked Customer Vehicle and Item Vehicle Fitment rows. Exact verified matches pass silently; broad, universal, provisional, missing-data, and mismatch results warn and require an auditable override reason, while still allowing the technician to proceed. The service stores a live pre-tax total, cost total, gross margin, and margin percentage. Historical subcontract rows remain auditable only through hidden, read-only legacy tables.
