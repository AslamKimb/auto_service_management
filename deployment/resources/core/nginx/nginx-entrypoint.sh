...
if [[ -z "$UPSTREAM_WAIT_TIMEOUT" ]]; then
  echo "UPSTREAM_WAIT_TIMEOUT defaulting to 120"
  export UPSTREAM_WAIT_TIMEOUT=120
fi

echo "Waiting for backend at ${BACKEND}"
wait-for-it -t "$UPSTREAM_WAIT_TIMEOUT" "$BACKEND"

echo "Waiting for websocket at ${SOCKETIO}"
wait-for-it -t "$UPSTREAM_WAIT_TIMEOUT" "$SOCKETIO"
