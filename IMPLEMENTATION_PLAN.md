...
**Legacy rollback contract (2026-07-26):** Before cycling the real stack, rollback analysis found that pre-correction images do not contain `/usr/local/bin/sync-assets`. Both Compose manifests therefore expose the same committed validator as a read-only fallback config while corrected images continue to use their self-contained `/opt/frappe-assets` seed and in-image executable. A disposable Compose project using the uncorrected `dev-81096a6` image exercised the fallback, synchronized all 60 manifest entries from that image's baked asset tree, and was removed with only its project-scoped test volume. The live `dms-image_*` volumes were not mounted or changed.
