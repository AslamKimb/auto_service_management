...
- [-] Package the built Frappe assets outside volume-masked paths and synchronize the derived assets volume before site bootstrap.
- [ ] Use `CUSTOM_IMAGE` as the single immutable image selector for every DMS service.
- [ ] Add focused synchronization and Compose-contract coverage, including fail-closed manifest validation.
- [ ] Verify fresh startup, repeated startup, old-to-new upgrade, and new-to-old rollback without deleting the existing `dms-image_*` volumes.
- [ ] Confirm all five apps, Car Workshop navigation, vehicle catalog, uploaded files, and business-record counts remain unchanged.
