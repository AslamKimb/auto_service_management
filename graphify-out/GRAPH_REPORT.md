# Graph Report - DMS  (2026-07-23)

## Corpus Check
- 192 files · ~83,853 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1934 nodes · 3026 edges · 331 communities (162 shown, 169 thin omitted)
- Extraction: 91% EXTRACTED · 9% INFERRED · 0% AMBIGUOUS · INFERRED: 260 edges (avg confidence: 0.77)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `df0202b1`
...
6. `TestPhase6Contracts` - 26 edges
7. `TestRepairJobWorkflowIntegration` - 26 edges
8. `get_service_components()` - 24 edges
9. `_create_repair_job()` - 23 edges
10. `Real-World Workflows — Auto Service Management` - 23 edges
...
Cohesion: 0.05
Nodes (73): _component_description(), _component_trace_fields(), create_material_request(), create_project_for_repair_job(), create_quotation(), create_sales_invoice(), create_sales_order(), create_stock_entry_for_material_issue() (+65 more)
...
Cohesion: 0.1
Nodes (43): recompute_repair_job_state(), sync_repair_job_related_tables(), approve(), reject(), _all_billable_components_submitted(), _assert_invoice_cancellation_allowed(), cancel_material_request(), cancel_sales_invoice() (+35 more)
...
Cohesion: 0.25
Nodes (9): _append_pending_labour_line(), _insert_authorization(), _insert_diagnosis(), _insert_quality_check(), _insert_road_test(), _insert_walkaround(), TestCustomerAuthorization, TestDiagnosisReport (+1 more)
...
Cohesion: 0.1
Nodes (10): _create_repair_job(), Create a Draft Repair Job., Create a Draft Repair Job., Create a Draft Repair Job., Create a Draft Repair Job., Create a Draft Repair Job., Create a Draft Repair Job., TestFleetServiceCampaign (+2 more)
...
Cohesion: 0.1
Nodes (20): Create Stock Entry (Material Issue) for requested Parts lines., Return Parts lines where issued_qty < quantity (shortage)., Return Parts lines where issued_qty < quantity (shortage)., Return stock components where issued_qty < quantity., Return stock components where issued_qty < quantity., Return stock components where issued_qty < quantity., Return stock components where issued_qty < quantity., Return stock components where issued_qty < quantity. (+12 more)

### Community 21 - "Community 21"
Cohesion: 0.1
Nodes (20): Create an idempotent Service History snapshot., Create an idempotent Service History snapshot., Create an idempotent Service History snapshot., Create an immutable Repair Job Log entry., Issue a Gate Pass for this Repair Job., Issue a Gate Pass for this Repair Job., Create an idempotent Service History snapshot., Create an idempotent Service History snapshot. (+12 more)
...
Cohesion: 0.1
Nodes (19): Auto Service Management Implementation Plan, Evidence Rules, Phase 0 — Repository Governance, Phase 10 — Repair Job State Authority Reconciliation, Phase 11 — Repair Job Component Billing and Single-Status Model, Phase 12 — Labour Service Items, Bay Warehouses, and Related-Table Consistency, Phase 13 — Workshop Operations, Release Policy, and Permissions, Phase 14 — Suite Integration: HRMS and Uganda Compliance (+11 more)

### Community 30 - "Community 30"
Cohesion: 0.11
Nodes (19): Auto Service Management, code:bash (git clone https://github.com/your-org/auto_service_managemen), code:bash (# Windows (run PowerShell as Administrator)), code:bash (docker compose -f docker-compose.dev.yml up -d), code:bash (curl http://auto-service.localhost:8080/api/method/ping), code:bash (docker exec dms-backend-1 bench --site auto-service-test.loc), code:bash (docker exec dms-backend-1 bench --site auto-service-test.loc), code:bash (docker exec dms-backend-1 bench --site auto-service.localhos) (+11 more)
...
Cohesion: 0.14
Nodes (5): IntegrationTestCase, TestERPNextAdapters, TestQualityCheck, TestRepairJobOverride, TestWorkshopBay
...
Cohesion: 0.21
Nodes (10): _build_sidebar_link(), ensure_permission(), _ensure_workspace_label(), _get_workspace_sidebar_items(), Desk desktop visibility for the Car Workshop entry., Permission check for the ``add_to_apps_screen`` hook.  	Any non-Guest user may s, Permission check for the ``add_to_apps_screen`` hook.  	Any non-Guest user may s, Permission check for the ``add_to_apps_screen`` hook.  	Any non-Guest user may (+2 more)
...
Cohesion: 0.18
Nodes (10): Run all desktop setup steps: icon, workspace fields, sidebar.  	Called from ``af, Run all desktop setup steps: icon, workspace fields, sidebar.  	Called from ``af, Run all desktop setup steps: icon, workspace fields, sidebar.  	Called from ``af, Run all desktop setup steps: icon, workspace fields, sidebar.  	Called from ``af, Run all desktop setup steps: icon, workspace fields, sidebar.  	Called from ``af, Run all desktop setup steps: icon, workspace fields, sidebar.  	Called from ``af, Run all desktop setup steps: icon, workspace fields, sidebar.  	Called from ``af, Run all desktop setup steps: icon, workspace fields, sidebar.  	Called from ``af (+2 more)
...
Cohesion: 0.33
Nodes (6): _ensure_workspace_app_field(), Create the App-type Desktop Icon if it does not already exist.  	Called from `, Set the ``app`` field on the Workspace if it is NULL.  	Required for Frappe v16, Set the ``app`` field on the Workspace if it is NULL.  	Required for Frappe v16, Set the ``app`` field on the Workspace if it is NULL.  	Required for Frappe v16, Set the ``app`` field on the Workspace if it is NULL.  	Required for Frappe v16

### Community 77 - "Community 77"
Cohesion: 0.33
Nodes (6): _ensure_workspace_type_field(), Set the ``type`` field on the Workspace if it is NULL.  	Required for Frappe v16, Set the ``type`` field on the Workspace if it is NULL.  	Required for Frappe v16, Set the ``type`` field on the Workspace if it is NULL.  	Required for Frappe v16, Set the ``type`` field on the Workspace if it is NULL.  	Required for Frappe v16, Set the ``type`` field on the Workspace if it is NULL.  	Required for Frappe v16

### Community 78 - "Community 78"
Cohesion: 0.33
Nodes (6): _ensure_workspace_sidebar(), Create the Workspace Sidebar entry if it does not exist.  	Frappe v16's desk loa, Rebuild the app-owned Workspace Sidebar for the workshop workspace.  	Frappe v16, Rebuild the app-owned Workspace Sidebar for the workshop workspace.  	Frappe v16, Rebuild the app-owned Workspace Sidebar for the workshop workspace.  	Frappe v16, Rebuild the app-owned Workspace Sidebar for the workshop workspace.  	Frappe v16
...
## Knowledge Gaps
- **679 isolated node(s):** `Set the ``app`` field on the Workspace if it is NULL.  	Required for Frappe v16`, `Set the ``type`` field on the Workspace if it is NULL.  	Required for Frappe v16`, `Keep the underlying workspace doc stable but expose a shorter label.`, `Rebuild the app-owned Workspace Sidebar for the workshop workspace.  	Frappe v16`, `Expose Car Workshop as one directly routed Desk icon.  	Called from ``after_inst` (+674 more)
...
- **Why does `RepairJob` connect `Community 0` to `Community 5`, `Community 10`, `Community 11`, `Community 19`, `Community 20`, `Community 21`, `Community 26`, `Community 31`?**
  _High betweenness centrality (0.116) - this node is a cross-community bridge._
- **Why does `iter_repair_job_components()` connect `Community 3` to `Community 0`, `Community 1`, `Community 2`, `Community 4`, `Community 5`, `Community 7`, `Community 8`, `Community 15`, `Community 18`?**
  _High betweenness centrality (0.110) - this node is a cross-community bridge._
- **Why does `TestPhase6Contracts` connect `Community 35` to `Community 5`, `Community 106`, `Community 13`, `Community 50`, `Community 87`, `Community 56`, `Community 57`?**
...
- **What connects `Set the ``app`` field on the Workspace if it is NULL.  	Required for Frappe v16`, `Set the ``type`` field on the Workspace if it is NULL.  	Required for Frappe v16`, `Keep the underlying workspace doc stable but expose a shorter label.` to the rest of the system?**
